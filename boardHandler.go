...
func GetBoard() http.Handler {
	return http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			selectAllBoardQuery := fmt.Sprintf("SELECT postId, title, writer, content FROM post limit %d, %d", page*10, page*10+limit)
			...
			for rows.Next() {
				var post model.Post
				_ = rows.Scan(&post.PostId, &post.Title, &post.Writer, &post.Content)
